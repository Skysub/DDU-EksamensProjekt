class TextField {
  ...
  TextField(PApplet program, String s, PVector pos, PVector size, boolean kunInt) {
    ...
    cp5.setFont(font);
    if (kunInt) textfield = cp5.addTextfield("StringTextField").setPosition(pos.x, pos.y).setSize(int(size.x), int(size.y)).setAutoClear(false).setText(s).setCaptionLabel("").keepFocus(false).setInputFilter(ControlP5.INTEGER);
    else textfield = cp5.addTextfield("StringTextField").setPosition(pos.x, pos.y).setSize(int(size.x), int(size.y)).setAutoClear(false).setText(s).setCaptionLabel("").keepFocus(false);
  }
