class BanePopUp {
  ...
  void Draw(boolean done, String[] time, boolean newRecord, String un) {
    ...
    if (done) { //Til hvis banen er færdig
      ...
      textAlign(LEFT, TOP);
      text("Congratulations! Level "+ levelNr +" cleared.", 15, 10);
      textSize(22);
      textAlign(CENTER);
      text("Press '  ' to play the level again.", width/(2*size), 280);
      fill(255, 50, 50);
      text("R", width/(2*size) - 92, 280);
    ...
    textAlign(CENTER);
    if (un == null) text("You are not logged in so your score won't be saved", width/(2*size), 310); 
    popMatrix();
    drawButtons(done);
  }
